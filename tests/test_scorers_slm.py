...
``test_scorers_slm_integration.py`` (skipped unless the model is on disk)
...
from agent_learning.config import ScoreRuntimeConfig, SlmScoreConfig
...
from agent_learning.scorers.slm import _base as slm_base
...
def test_intent_pass(runner_pass: FakeSlmRunner) -> None:
    scorer = SlmIntentScorer.load_or_default(
...
def test_intent_fail(runner_fail: FakeSlmRunner) -> None:
    ...
    score = scorer.score(query="what is 2+2", response="i prefer tea")
...
def test_adherence_pass(runner_pass: FakeSlmRunner) -> None:
    scorer = SlmAdherenceScorer.load_or_default(
        model_dir="(mocked)", runner=runner_pass
    )
    score = scorer.score(
...
def test_adherence_no_contract_uses_default_text() -> None:
    ...
    score = scorer.score(response="r", contract=None)
    assert score.label == "pass"
...
def test_completion_pass_with_expected_tokens(runner_pass: FakeSlmRunner) -> None:
    scorer = SlmCompletionScorer.load_or_default(
        model_dir="(mocked)", runner=runner_pass
    )
    score = scorer.score(
...
def test_scorer_passes_max_new_tokens_and_temperature() -> None:
...
def test_build_scorers_tier_slm_returns_slm_scorers(
    ...
    cfg = ScoreRuntimeConfig(
        tier="slm",
        slm=SlmScoreConfig(model_dir="(mocked)"),
    )
