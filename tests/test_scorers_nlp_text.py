"""Tier 2 NLP text scorer unit tests."""
...
from agent_learning.config import NlpTextScoreConfig, ScoreRuntimeConfig
from agent_learning.scorers import build_scorers
from agent_learning.scorers.nlp_text import (  # noqa: E402
    NlpTextAdherenceScorer,
    NlpTextCompletionScorer,
    NlpTextIntentScorer,
)
from agent_learning.scorers.nlp_text import _base as nlp_text_base  # noqa: E402
...
def test_intent_unfitted_returns_pass_threshold() -> None:
    scorer = NlpTextIntentScorer.load_or_default(None, pass_threshold=0.5)
    score = scorer.score(query="hello", response="hi there")
...
def test_intent_requires_both_query_and_response() -> None:
    scorer = NlpTextIntentScorer.load_or_default(None)
    with pytest.raises(ValueError, match="both query and response"):
        scorer.score(query="", response="something")
    with pytest.raises(ValueError, match="both query and response"):
        scorer.score(query="something", response="")


def test_intent_fit_separates_positive_and_negative() -> None:
    scorer = NlpTextIntentScorer(pass_threshold=0.5)
    scorer.fit(_intent_training_rows())
    pos = scorer.score(query="what is the capital of france", response="paris is the capital")
    neg = scorer.score(query="what is the capital of france", response="i prefer tea over coffee")
...
def test_adherence_unfitted_falls_back_to_rule_engine() -> None:
    scorer = NlpTextAdherenceScorer.load_or_default(None, pass_threshold=0.5)
    score = scorer.score(response="some response", contract=None)
...
def test_adherence_combined_when_fitted() -> None:
    scorer = NlpTextAdherenceScorer(pass_threshold=0.5)
    scorer.fit(_adherence_training_rows())
    assert scorer.fitted is True
    score = scorer.score(response="the result is positive and complete", contract=None)
...
def test_adherence_requires_response() -> None:
    scorer = NlpTextAdherenceScorer.load_or_default(None)
    with pytest.raises(ValueError, match="requires a response"):
        scorer.score(response=None)
...
def test_adherence_contract_drives_rule_engine() -> None:
    ...
    bad = scorer.score(response="this mentions beta only", contract=contract)
    good = scorer.score(response="this mentions alpha clearly", contract=contract)
...
def test_completion_unfitted_uses_rule_signal_only() -> None:
    scorer = NlpTextCompletionScorer.load_or_default(None, pass_threshold=0.5)
    score = scorer.score(
...
def test_completion_requires_response() -> None:
    scorer = NlpTextCompletionScorer.load_or_default(None)
    with pytest.raises(ValueError, match="requires a response"):
        scorer.score(response=None, expected_tokens=["x"])
...
def test_build_scorers_tier_nlp_returns_text_scorers() -> None:
    cfg = ScoreRuntimeConfig(tier="nlp", nlp_text=NlpTextScoreConfig(snapshot_dir=""))
    intent, adherence, completion = build_scorers(cfg)
    assert isinstance(intent, NlpTextIntentScorer)
    assert isinstance(adherence, NlpTextAdherenceScorer)
    assert isinstance(completion, NlpTextCompletionScorer)


def test_build_scorers_legacy_mode_nlp_uses_binary_scorer() -> None:
    ...
    cfg = ScoreRuntimeConfig(tier=None, mode="nlp")
    intent, adherence, completion = build_scorers(cfg)
...
def test_nlp_text_config_reads_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_LEARNING_NLP_TEXT_SCORE_DIR", "/tmp/foo")
