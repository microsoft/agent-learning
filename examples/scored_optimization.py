"""Scored optimization example: learn from the SDK's own tiered scorers.

``quickstart.py`` stubs the reward and ``next_best_action.py`` simulates
an outcome. This example closes the loop with the SDK's *real* scoring
layer: every episode is scored by the Tier 1 (pure standard library,
zero-dependency) scorers returned by
:func:`agent_learning.scorers.build_scorers`, and those scores -- not a
...
templates; the scorers decide how good each answer is:

    * ``template_rich``   -- names the ticket, resets the password, and
    walks the user through verification. Passes every scorer.
...
This single example exercises all six SDK building blocks:
    ...
    Scorers   build_scorers(tier="stdlib") scores each episode.
    Metrics   ScoreResult -> MetricResult bridges scorers into the reward.
    ...
    Episodes  full Episode records carry the contract, expected tokens,
              tool calls, and conversation history the scorers read.

Run with ``python examples/scored_optimization.py``. It uses the
in-memory store and the stdlib scorers, so it needs no Azure credentials
...
from agent_learning.scorers.base import Scorer, ScoreResult
...
def contract_for(ticket: str) -> Dict[str, object]:
    """The per-episode adherence contract the scorer enforces."""
    return {
...
# ---------------------------------------------------------------------------
# 3. Scorers -- the SDK's Tier 1 stdlib trio. Adherence and completion are
...
#    unfitted default.
# ---------------------------------------------------------------------------
def build_example_scorers() -> Tuple[Scorer, Scorer, Scorer]:
    """Build the stdlib scorers and fit the intent head on a tiny corpus."""
    intent, adherence, completion = build_scorers(ScoreRuntimeConfig(tier="stdlib"))
    ...
    intent.fit(corpus)  # type: ignore[attr-defined]  # StdlibIntentScorer.fit

    return intent, adherence, completion


# ---------------------------------------------------------------------------
# 4. Metrics bridge -- map each ScoreResult onto the MetricResult shape the
#    reward shaper already understands. This is the one adapter that lets
#    any scorer tier feed the existing reward pipeline unchanged.
# ---------------------------------------------------------------------------
def score_episode(
    scorers: Tuple[Scorer, Scorer, Scorer],
    episode: Episode,
) -> List[MetricResult]:
    """Score one episode with the three scorers -> three MetricResults."""
    ...
    scored: List[Tuple[MetricName, ScoreResult, str]] = [
    ...
    return [
        MetricResult(
            metric=name,
            score=score_result.confidence,
            normalized=score_result.normalized,
            status="completed",
            reason=score_result.label,
            properties=score_result.features,
            evaluator=f"tier1-stdlib:{scorer_name}",
        )
        for name, score_result, scorer_name in scored
    ]


# ---------------------------------------------------------------------------
# 5. Runner -- wire the scorers and the dormant shaping penalties into the
#    existing pipeline WITHOUT touching the SDK. We override two hooks:
#      * evaluate_episode  -> score with the scorers (instead of Azure).
#      * score_and_record  -> shape with routing + hallucination signals
#                             the default shape_episode_reward never passes.
# ---------------------------------------------------------------------------
class ScoredRunner(LearningRunner):
    """A LearningRunner whose reward comes from the stdlib scorers."""

    def __init__(self, *, scorers: Tuple[Scorer, Scorer, Scorer], **kwargs: object) -> None:
        super().__init__(**kwargs)  # type: ignore[arg-type]
        self._scorers = scorers

    def evaluate_episode(self, episode: Episode) -> List[MetricResult]:
        return score_episode(self._scorers, episode)
...
def main() -> None:
    ...
    runner = ScoredRunner(
        ...
        store=store,
        policy=policy,
        metrics=[],  # scorers are supplied via evaluate_episode instead
    ...
    rounds = 30
    per_round = 30
    print(f"\nTraining for {rounds} rounds x {per_round} episodes (stdlib scorers) ...")
    ...
    # Show the scorer decomposition for one episode of each template so the
    # reward gradient is auditable.
    print("\n=== Per-template scorer results (single ticket) ===")
    ticket = TICKETS[0]
    for action_id, template in TEMPLATES.items():
        ...
        results = score_episode(scorers, probe)
