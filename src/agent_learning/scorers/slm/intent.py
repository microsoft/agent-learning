"""Tier 3 SLM intent scorer.

Asks Phi-4-mini-instruct whether the model's response actually addresses
the user's question. The scorer requires both ``query`` and ``response``;
...
@dataclass
class SlmIntentScorer(SlmScorerBase):
    """Phi-4-mini-instruct intent scorer."""
    ...
    def _validate(self, **inputs: object) -> None:
        ...
        if not query or not response:
            raise ValueError(
                "intent SLM scorer requires both query and response"
            )
