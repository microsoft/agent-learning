"""Tier 1 stdlib task-completion scorer.

Token-coverage scorer with no training. Given a list of expected
tokens (or a single string that the scorer splits on whitespace),
...
matching the permissive default of the other Tier 1 scorers.
"""
...
@dataclass
class StdlibCompletionScorer:
    """Stdlib token-coverage completion scorer."""
    ...
    def score(
        ...
        **_: object,
        ...
        if response is None:
            raise ValueError(
                "StdlibCompletionScorer requires response"
            )
        targets: List[str] = []
        ...
        if not targets:
            ...
            for tok in targets:
                ...
                if " " not in normalized:
                    ...
                    # response. Keeps the scorer useful for short keyed
...
__all__ = ["StdlibCompletionScorer"]
