"""Tier 1 stdlib intent-resolution scorer.
...
The scorer has two states:

- **Unfitted** (initial). ``score()`` returns ``label="pass"`` with
    confidence 0.5. The unfitted scorer is intentionally permissive so
...
:class:`agent_learning.classifiers.scorers._base.BinaryScorer`, so
operators can persist and reload a fitted scorer via :meth:`save`
and :meth:`load`.
"""
...
@dataclass
class StdlibIntentScorer:
    """Stdlib bag-of-words intent scorer."""
    ...
    def load_or_default(
        ...
        *,
        ...
        if snapshot_dir:
            path = os.path.join(
                snapshot_dir, f"{scorer.name}.stdlib.json"
            )
            if os.path.isfile(path):
                with open(path, "r", encoding="utf-8") as fh:
                    scorer.load(json.load(fh))
        return scorer

    def fit(self, training_rows: List[dict]) -> "StdlibIntentScorer":
    ...
    def score(
        ...
        **_: object,
        ...
        if query is None or response is None:
            raise ValueError(
                "StdlibIntentScorer requires query and response"
            )
    ...
    def load(self, snapshot: dict) -> "StdlibIntentScorer":
...
# downstream debugging hooks that import StdlibIntentScorer.math.
_ = math

__all__ = ["StdlibIntentScorer"]
