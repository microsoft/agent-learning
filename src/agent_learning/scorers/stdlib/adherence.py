"""Tier 1 stdlib task-adherence scorer.

Deterministic rule engine over a per-task ``contract`` dict. No
training is required; the scorer runs synchronously against the response
...
``[0, 1]``. With no constraints in the contract the scorer returns
``1.0`` ("pass with full confidence"); this is the same permissive
default the other Tier 1 scorers use when unfitted.
"""
...
@dataclass
class StdlibAdherenceScorer:
    """Stdlib rule-engine adherence scorer."""
    ...
    def load_or_default(
        ...
        *,
        ...
        """Match the load_or_default contract of the other scorers.
    ...
    def score(
        ...
        **_: object,
        ...
        if response is None:
            raise ValueError(
                "StdlibAdherenceScorer requires response"
            )
