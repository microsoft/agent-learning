"""Tier 2 NLP text adherence scorer.

Combines the deterministic rule engine that ships in
:class:`agent_learning.scorers.stdlib.StdlibAdherenceScorer` with a
...
from ..stdlib.adherence import StdlibAdherenceScorer
from ._base import _NlpTextScorerBase


@dataclass
class NlpTextAdherenceScorer(_NlpTextScorerBase):
    """Adherence scorer backed by TF-IDF + a deterministic rule engine."""
    ...
    def score(
        ...
        **_: object,
        ...
        if response is None:
            raise ValueError(
                "adherence NLP text scorer requires a response"
            )
        # Rule engine via the Tier 1 backend.
        rule_scorer = StdlibAdherenceScorer(pass_threshold=self.pass_threshold)
        rule_score = rule_scorer.score(response=response, contract=contract)
...
__all__ = ["NlpTextAdherenceScorer"]
