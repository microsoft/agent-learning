...
the SDK :class:`JudgeScore` contract.
...
class _LlmJudgeWrapper:
    """Common base for the three concrete LLM judges."""

    cfg: JudgeConfig
    name: str = "llm-judge"
    label_name: str = "llm-judge"
    evaluator_attr: str = ""
    ...
    def _evaluator(self) -> Any:
        ...
        try:
            ...
            raise ImportError(
                "LLM judges require the optional 'azure-ai-evaluation' package. "
    ...
    def score(
        ...
        **kwargs: object,
        ...
        """Run the underlying evaluator and project to a JudgeScore.
        ...
        if query is None or response is None:
            raise ValueError(
                f"{self.name} LLM judge requires query (or request) and response"
            )
...
def _project_to_score(
    ...
    """Map an evaluator return value onto a :class:`JudgeScore`.
