"""Phi-4-mini-instruct ONNX runner + shared judge base for Tier 3 SLM.
...
The runner abstraction also makes the per-judge prompt building and JSON
...
Each subclass of :class:`SlmJudgeBase` supplies its own ``system`` and
...
class SlmRunner:
    ...
    def get_shared(cls, model_dir: str) -> "SlmRunner":
        ...
        the three Tier 3 judges keeps inference hot and avoids the
        re-loading cost.
        """
...
@dataclass
class SlmJudgeBase:
    """Shared scoring scaffolding for Tier 3 small-language-model judges."""
    ...
    def _score_from_inputs(self, **inputs: Any) -> JudgeScore:
