...
@dataclass
class SlmCompletionJudge(SlmJudgeBase):
    """Phi-4-mini-instruct completion judge."""
    ...
    def _validate(self, **inputs: object) -> None:
        if inputs.get("response") is None:
            raise ValueError(
                "completion SLM judge requires a response"
            )
