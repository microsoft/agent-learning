...
* Ship a default scorer implementation that runs without an external service when the caller chooses one of the local tiers.
* Preserve the existing LLM scorer path as a first-class tier.
...
* We are not removing the LLM backend. Callers who prefer the LLM scorer keep it.
...
* We are not training the three scorers jointly. Each scorer is independent and snapshot-versioned on its own.

## The mode switch

The SDK exposes a preferred `score_tier` selector and a compatibility `score_mode` axis through the configuration, factory, and snapshot metadata.

The runtime configuration carries these selectors and backend blocks:

* `score_tier`, one of `"stdlib"`, `"nlp"`, `"slm"`, or `"llm"`. The environment variable `AGENT_LEARNING_SCORE_TIER` selects it. When unset, the factory falls back to `score_mode`.
* `score_mode`, one of `"nlp"` or `"llm"`. The environment variable `AGENT_LEARNING_SCORE_MODE` overrides the in-code default. When unset, the mode is `"llm"` to preserve existing behavior.
* Tier-specific blocks: `StdlibScoreConfig`, `NlpScoreConfig`, `NlpTextScoreConfig`, `SlmScoreConfig`, and `ScoreConfig` for the LLM stack. All blocks always exist; only the active one is consulted.

A single factory entry point, `agent_learning.scorers.build_scorers`, hides the backend choice. The factory inspects the tier, mode, installed extras, and configuration, then returns the three scorers (intent, adherence, completion) bound to the appropriate tier. Callers never instantiate a scorer directly.
...
> [!NOTE]
> Sizes are wheel-on-disk plus transitive runtime dependencies, rounded. Latencies are p99 per evaluation on a 4-vCPU CPU-only worker with the response held under 4 KB of text.
...
Tier 1 handles the easy bands of all three scorers: closed-set intent classification with small label inventories, hard-rule contract checks (length, required substrings, forbidden substrings, JSON-shape via stdlib `json`), and required-token coverage measured by exact and case-insensitive matching.
...
Tier 3 makes the scorers robust to paraphrase. Intent gains per-class centroids and cosine similarity; adherence gains semantic detection of "did the response say what the contract required even if it used different words"; completion gains semantic coverage of required artifacts.
...
## Per-scorer tier mapping

Each scorer has a recipe at every tier. The factory picks the highest tier whose extras are installed.

| Scorer | Tier 1 (stdlib) | Tier 2 (NLP library) | Tier 3 (small LM) | Tier 4 (LLM) |
|---|---|---|---|---|
...
Two environment variables mirror these fields so the same SDK build can switch between developer-laptop, CI, and production AKS without code changes: `AGENT_LEARNING_SCORE_CREDENTIAL_MODE` and `AGENT_LEARNING_SCORE_USER_ASSIGNED_CLIENT_ID`.
...
Every reward record persisted to Cosmos already carries an immutable lineage block. The scoring layer adds:

* `score_mode`, the active mode at scoring time.
* `score_tier`, the resolved tier (1, 2, 3, or 4).
...
* Calibration: per-scorer F1 within 5 percentage points of Tier 4 across the validation set, with the operating threshold tuned per tier.
...
|---|---|---|
| P1 | Tier 1 (stdlib) | `score_mode` plumbing, `ScoreRuntimeConfig`, `build_scorers` factory, stdlib intent and adherence and completion scorers, full test coverage. Default mode stays `"llm"`. |
| P2 | Tier 2 (NLP library) | `[nlp]` extra, TF-IDF and calibrated soft heads for all three scorers, snapshot serialization, offline tuner integration. |
...
| P4 | Tier 4 (LLM) plus managed identity | Tier 4 already exists; this phase adds `credential`, `credential_mode`, and `user_assigned_client_id` to `ScoreConfig`, wires `DefaultAzureCredential` and `ManagedIdentityCredential` and `WorkloadIdentityCredential`, and documents the AKS workload-identity recipe. |
