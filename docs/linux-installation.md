...
```bash
sudo bash ./scripts/install-linux.sh --version 0.9.1 --install-dir /usr/local/bin
```
...
docker run --rm --entrypoint /bin/bash ubuntu:24.04 -lc '
  ...
  curl -fsSL -o /tmp/agent-learning.tar.gz https://github.com/microsoft/agent-learning/releases/download/v0.9.1/agent-learning-cli-0.9.1-linux-x64.tar.gz
